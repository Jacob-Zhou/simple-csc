...
def correct_sentence(sentence, prompt, lmcsc_model, streamer):
    batch = [sentence]
    ...
    (
        ...
        beam_scorer,
        observed_sequence_generator,
    ) = lmcsc_model.preprocess(batch, contexts, prompt_split="")
    ...
    threading.excepthook = partial(excepthook, streamer=streamer)

    thread = Thread(
        target=lmcsc_model.model.distortion_guided_beam_search, kwargs=generation_kwargs
    )
    thread.start()
...
def correct_sentences(obversed_text, prompt, lmcsc_model):
    ...
    streamer = BeamStreamer(lmcsc_model.tokenizer, skip_special_tokens=True)
    ...
    for sentence in sentences:
        ...
        changes = correct_sentence(sentence, prompt_context, lmcsc_model, streamer)
        ...
        with result_placeholder:
            for new_text in streamer:
                ...
                if oom_error:
                    break
                output = [new_text[len(prompt_context) :]]
                output = rebuild_sentences(output, changes)[0]

                # Handle oov characters
                output = "".join(
                    [
                        o_char if o_char != OOV_CHAR else s_char
                        for o_char, s_char in zip(output, sentence)
                    ]
                )
...
def load_model(selected_model):
    lmcsc_model = LMCSCModel(
        ...
        shape_similar_threshold=st.session_state["default_shape_similar_threshold"],
...
config = yaml.safe_load(open("demo_config.yaml", "r"))
